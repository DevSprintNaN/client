      ...
      <main className="container mx-auto max-w-screen-lg h-full">
        {!uploading && (<article aria-label="File Upload Modal" className="relative h-full flex flex-col bg-white shadow-xl rounded-md" onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} onDragEnter={(e) => e.preventDefault()} onDragLeave={(e) => e.preventDefault()}>
          ...
          <footer className="flex justify-center space-x-2 px-8 pb-8 pt-4">
            <div className={`text-red-500 text-center transition-colors duration-300 bg-gray-200 hover:bg-red-500 hover:text-white py-2 px-6 md:px-12 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 md:mb-0 md:mr-2 hover:cursor-pointer font-medium ${uploading ? "opacity-50 cursor-not-allowed" : ""}`} onClick={handleCancel}>Cancel</div>

            <button className={` bg-purple-700 text-white py-2 px-6 md:px-12 rounded-md hover:bg-purple-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 transition-colors duration-300 ${uploading ? "opacity-50" : ""}`} disabled={uploading} onClick={() => handleUpload()}>Upload</button>
          </footer>
          ...
          <section className="h-full overflow-auto p-8 w-full flex flex-col">
            ...
            <div className="flex flex-col items-center justify-center">
              <div className="w-full bg-violet-200 rounded-full h-2.5">
                <div className="bg-purple-700 h-2.5 rounded-full" style={{ width: progress.toPrecision(3) + '%' }}></div>
              </div>
