...
const ViewForums = () => {
    const { loading,handleSearch,searchItems,search,setSearch } = useFetchForums();
    const { currentPage, setCurrentPage, totalPages, getCurrentItems, handlePageChange } = usePagination(12, searchItems);
        ...
        return (
            <div className={`h-full p-2 bg-violet-100 w-full min-h-screen`} >
                ...
                <div className="text-gray-900 pb-8 pr-0 pl-0 bg-white">
                    ...
                    <div className="w-full content-center mb-3 p-3 flex justify-center items-center">
                        <input type="text" placeholder="Search forums" className="w-3/4 p-2 border rounded-md focus:border-gray-200 focus:outline-none transition-colors duration-300" value={search} onChange={handleSearch} />
                    </div>
